...
class Tps extends MY_Controller
{
    ...
    public function data($param = "")
    {
        ...
        $lat_setting = $pengaturan['center_map_lat'];
        $lng_setting = $pengaturan['center_map_lng'];

        $tps = $this->tpsModel->getDataID($param);
        $lat = isset($tps['lat']) ? $tps['lat'] : '';
        $lng = isset($tps['lng']) ? $tps['lng'] : '';


        $this->db->get('tb_pengaturan')->row_array();

        $config = [
            'map_div_id' => "map-add",
            'map_height' => "600px",
            'center' => (!empty($lat) && !empty($lng)) ? $lat . "," . $lng : $lat_setting . ',' . $lng_setting,
        ...
        $marker = [
            'position' => (!empty($lat) && !empty($lng)) ? $lat . "," . $lng : $lat_setting . ',' . $lng_setting,
            'draggable' => true,
            'animation' => 'BOUNCE',
    ...
    public function save($param = "")
    {
        if ($this->input->is_ajax_request()) {
            ...
            $validation->set_rules("kapasitas", "Kapasitas", "trim|required", [
                'required' => "{field} tidak boleh kosong",
            ]);
    ...
    public function gantiStatus($id_tps)
    {
        if ($this->input->is_ajax_request()) {

            $id_tps = decode($id_tps);
            $csrf = csrf_hash();
            if ($id_tps) {
                ...
                $query = $this->db->where("id_tps", $id_tps)->update("tb_tps", $data);

                if ($query) {
                    $msg = ['csrf' => $csrf, 'success' => true, 'pesan' => "Status TPS berhasil diperbarui!"];
                } else {
                    $msg = ['csrf' => $csrf, 'success' => false, 'pesan' => "Status TPS gagal diperbarui!"];
                }
